...
select O.OrderID, C.FirstName, O.OrderDate from Customers as C
...
select D.DepartmentName,E.Name,E.Salary from Departments as D
...
select C.FirstName, O.OrderID, O.OrderDate,C.Country from Customers as C
inner join Orders as O
on C.CustomerID=O.CustomerID 
...
select C.FirstName,isnull(total_orders ,0) as total_orders from Customers as C
inner join (select CustomerID,count(OrderID) as total_orders from Orders group by CustomerID) as O
on C.CustomerID=O.CustomerID
...
select C.FirstName,O.MostRecentOrderDate from Customers as C
left join (select CustomerID,max(OrderDate)as MostRecentOrderDate from Orders group by CustomerID) as O
on C.CustomerID=O.CustomerID
...
select ProductName, TotalSalesAmount from Products as P
inner join (select ProductID,sum(SaleAmount) as TotalSalesAmount  from Sales group by ProductID) as S
on P.ProductID=S.ProductID
...
SELECT c.CategoryName, ProductCount from Categories as C
inner join ( select Category,count(ProductName) as ProductCount from Products group by Category)as p
...
inner join Departments as D
on E.DepartmentID=D.DepartmentID
where D.DepartmentName='Human Resources' or D.DepartmentName='Finance' or (LEN(E.Name) - LEN(REPLACE(LOWER(E.Name), 'a', '')) +
...
select P.ProductName,P.Price,QuantitySold from Products as P
inner join (select ProductID,count(SaleID) as QuantitySold from Sales group by ProductID) as S
on P.ProductID=S.ProductID
